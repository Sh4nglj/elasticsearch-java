...
/**
 * Weighted round-robin request routing strategy.
 */
public class WeightedRoundRobinRoutingStrategy implements RequestRoutingStrategy {
    ...
    public Iterator<Node> selectNodes(
        ...
        if (!livingNodes.isEmpty()) {
            ...
            if (!selectedLivingNodes.isEmpty()) {
                 ...
                 */
                List<Node> weightedNodes = new ArrayList<>();
                for (Node node : selectedLivingNodes) {
                    NodeMetrics metrics = stats.getNodeMetrics(node.getHost());
                    // Update weight based on current metrics
                    metrics.updateWeightBasedOnMetrics();
                    int weight = (int) Math.round(metrics.getWeight() * 10); // Scale weight to integer
                ...
                if (!weightedNodes.isEmpty()) {
                    // Rotate based on current weight counter
                    long current = currentWeight.getAndIncrement();
                    Collections.rotate(weightedNodes, (int) (current % weightedNodes.size()));
                    return weightedNodes.iterator();
                }
            }
        }
